...
output_name_quant <- "20250327_quant_FC_16O_Wk52.csv"
output_name_metadata <-"20250327_md_FC_16O_Wk52.csv"

#Filter metadata
colnames(metadata)<-gsub("ATTRIBUTE_","",colnames(metadata)) #Remove "ATTRIBUTE_"
colnames(metadata)[1] <- "filename"
metadata <- metadata %>%
  dplyr::filter(metadata$Sample_Type == "Frontal Cortex" & metadata$Isotope == "16O" & metadata$Wk == "52-56")
...
colnames(quant_new)<-gsub(".mzML"," Peak area",colnames(quant_new)) #Add back " Peak area"
